...
vessl experiment create \
  ...
  --resource "gpu-${GPUS}" \
  --command "bash -x scripts/install.sh && bash -x scripts/deardr/pretrain.sh ${GPUS} \$reader \$learning_rate \$batch_size \$steps \$eval_freq \$lr_scheduler_type \$val_reader \$train_db \$val_db \$train_file \$val_file \$model_name" \
