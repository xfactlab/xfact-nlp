...
vessl experiment create \
  ...
  -h reader=$READER_TRAIN -h learning_rate=$LEARNING_RATE -h batch_size=8 -h steps=1 -h eval_freq=15 -h lr_scheduler_type=${LR_SCHEDULER} \
  -h val_reader=fever -h train_db=wiki-pretraining -h val_db=fever -h train_file=shuf_100k.jsonl -h val_file=shared_task_dev.jsonl \
