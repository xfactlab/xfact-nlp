...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, data_args, model_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
    ...
    if is_seq2seq:
        ...
        extra_kwargs["generation_max_length"] = data_args.max_target_length
    else: # not is_seq2seq:
        global_labels = defaultdict(int)
        extra_kwargs["label_dict"] = global_labels

    loaded_datasets = {
        split: dataset_classes[split](tokenizer,readers[split].read(path),max_seq_length,
                                               name=split,
