...
class XFactDataset(TorchDataset, Registrable, ABC):
    ...
    def collate_fn(model, batch, pad_token_id, ignore_pad_token_for_loss=True) -> Dict[str, torch.Tensor]:
        ...
        # decoder_input_ids = input_ids # add
        # decoder_input_ids = torch.stack([x["input_ids"] for x in batch]) #add
        masks = torch.stack([x["attention_mask"] for x in batch])
        # source_ids, source_mask = model.trim_seq2seq_batch(
        #     input_ids, pad_token_id, attention_mask=masks  # trim ->trim seq
        # )
        source_ids, source_mask = trim_batch(
...
class XFactSeq2SeqDataset(XFactDataset, ABC):
    ...
    def generate(self, instance) -> Dict[str, torch.Tensor]:
        ...
        # if not self.blind_test_mode: #add 뺌
        ...
        ret = {
            ...
            "decoder_input_ids": target_ids, #add for t5
        }
...
class XFactCausalDataset(XFactDataset, ABC):
    def __init__(
            self,
            tokenizer,
            instance_generator,
        ...
        super(XFactCausalDataset, self).__init__(tokenizer=tokenizer,
                                                  instance_generator=instance_generator,
                                                  max_source_length=max_source_length,
                                                  name=name,
                                                  n_obs=n_obs,
