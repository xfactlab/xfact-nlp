...
class HuggingFaceModelFactory(ModelFactory):
    ...
    def get_model(self,
        ...
        #     keytmp = list(loaded_datasets.keys())[0]
        #     is_seq2seq = isinstance(loaded_datasets.__getitem__(keytmp), XFactSeq2SeqDataset)
        # else:
        key_data = loaded_datasets.keys()
        if len(key_data) == 1:
            loaded = loaded_datasets.get(list(key_data)[0])
            is_seq2seq = isinstance(loaded, XFactSeq2SeqDataset)
        else:
