...
class ClassificationScorer(Scorer):
    def __call__(self, actual, predicted, **kwargs):
        return {
            "em": micro(exact_match, actual, predicted),
            "recall": micro(recall, actual, predicted)
        }
def micro(method, actual, predicted):
    return sum(method(a,p) for a, p in zip(actual,predicted) if a is not None) / len(actual)
...
def recall(actual, predicted):
    ...
    print(predicted_set)
    result_recall = (
        sum(1.0 for a in actual_set if a in predicted_set) / float(len(actual_set))
        if len(actual)
        else 1.0
    )
    print(result_recall)
